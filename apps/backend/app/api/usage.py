...
async def get_usage(organization_id: UUID, service: UsageService = Depends(get_usage_service)):
    return await service.get_usage(organization_id)


@router.post("/{organization_id}/channels/{channel_id}")
async def register_channel(organization_id: UUID, channel_id: UUID, service: UsageService = Depends(get_usage_service), auth: None = Depends(require_mutation_auth)):
    try:
