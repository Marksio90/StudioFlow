...
def create_video_project(payload: VideoProjectCreate, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    try:
...
def patch_video_project(project_id: UUID, payload: VideoProjectUpdate, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
    return service.update_project(project_id, payload)


@router.delete("/{project_id}", status_code=204)
def delete_video_project(project_id: UUID, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
...
def start_workflow(project_id: UUID, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
    return service.start_workflow(project_id)


@router.post("/{project_id}/request-approval")
def request_approval(project_id: UUID, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
    return service.request_approval(project_id)


@router.post("/{project_id}/approve")
def approve(project_id: UUID, payload: ApprovalDecisionIn, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
    return service.approve(project_id, payload.comment, payload.decided_by_user_id)


@router.post("/{project_id}/reject")
def reject(project_id: UUID, payload: ApprovalDecisionIn, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
    return service.reject(project_id, payload.comment, payload.decided_by_user_id)


@router.post("/{project_id}/needs-changes")
def needs_changes(project_id: UUID, payload: ApprovalDecisionIn, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
...
def run_compliance(project_id: UUID, payload: dict, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
...
def create_analytics_snapshot(project_id: UUID, payload: AnalyticsSnapshotIn, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
    _require_project(service, project_id, correlation_id)
...
def create_publishing_plan(payload: PublishingPlanCreate, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
...
def schedule_publishing(plan_id: UUID, payload: PublishingPlanSchedule, service: VideoProjectService = Depends(get_video_project_service), correlation_id: str = Depends(get_correlation_id), auth: None = Depends(require_mutation_auth)): 
